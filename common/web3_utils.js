...
const noiaTokenDecimals = new BigNumber('1e+18');
//const decimals = await this.instances.tokenContract.decimals.call();

function noiaTokensToWeis(tokens) {
  const bn = new BigNumber(tokens);
  const tokensInWeis = bn.times(noiaTokenDecimals);
  return tokensInWeis;
}

function noiaTokensFromWeis(weis) {
  const bn = new BigNumber(weis);
  return bn.dividedBy(noiaTokenDecimals);
}
...
module.exports = {
  ...
  noiaTokensToWeis: noiaTokensToWeis,

  noiaTokensFromWeis: noiaTokensFromWeis,

  noiaTokenDecimals: noiaTokenDecimals
