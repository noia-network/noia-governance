...
// const ERC223NoFallback = artifacts.require('ERC223NoFallback');
// const ERC223WithFallback = artifacts.require('ERC223WithFallback');
...
contract('ERC777 Compliance Test', function (accounts) {
  let tokenContract;
  let acc0 = accounts[0];
  let acc1 = accounts[1];
  ...
  before(async function () {
    ...
    tokenContract = ERC20Token.at(await noia.tokenContract.call());
    console.log(`NOIA Token address: ${tokenContract.address}`);
  });
  ...
  it('transfer 100 sample tokens to acc1', async function() {
    ...
    console.log(`Transfering 100 token from account0 to account1 ... Old balance: ${oldBalance}`);
